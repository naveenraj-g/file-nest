 ...
 *
 * Server component: resolves the project and the current org, fetches initial
 * API key list from the IAM, and passes all data down to the client table.
 *
 * @module
 */
...
export default async function ProjectApiKeysPage({ params }: Props) {
  ...
  const [[project], [keysData]] = await Promise.all([
    ...
    listApiKeysAction({ payload: { organizationId, projectId } }),
  ]);

  const initialData = keysData ?? { apiKeys: [] };
    ...
    <div className="space-y-6">
      <div>
        ...
        <p className="text-sm text-muted-foreground mt-0.5">
          Keys are scoped to{" "}
          <strong className="text-foreground">{project?.name ?? projectId}</strong>. Each key
          carries explicit scopes that limit what it can do.
        </p>
      </div>
