...
Every request carries an `x-filenest-signature` header â€” an HMAC-SHA256 hex digest of the raw request body, keyed with your webhook's `signing_secret`.

```typescript
import { createHmac, timingSafeEqual } from "crypto";

export function verifyWebhookSignature(
  body: string,           // raw request body â€” do NOT parse before verifying
...
`signing_secret` is returned exactly once on creation. Store it securely â€” it cannot be retrieved again.
</Callout>
...
**Response `200`:** updated webhook object (no `signing_secret` field â€” secret does not rotate on update).
