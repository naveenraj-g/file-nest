...
The storage server (MinIO/RustFS) uses its configured KMS key to encrypt the object. The encryption happens server-side â€” FileNest never holds the encryption key.
...
Or via the Console: **Project Settings â†’ Storage â†’ Encryption at rest** toggle.
