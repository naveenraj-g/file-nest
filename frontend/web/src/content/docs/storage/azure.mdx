...
2. The storage account's **access key** (found under Storage Account â†’ Access keys in the Azure portal)
...
The storage account key grants full read/write access to all containers in the account. For least-privilege access, consider using a **SAS token** or **Managed Identity** â€” contact FileNest support for those options.
