...
<Callout type="note">
The `endpoint_url` must be reachable from the FileNest API server. Self-signed TLS certificates are not supported â€” use a CA-signed cert or disable TLS at the endpoint and use HTTP only.
</Callout>
...
Enable SSE via the Console (Project Settings â†’ Storage â†’ Encryption) or the API:
