...
RustFS SSE is **off by default** and can be toggled per project. The setup is identical to [MinIO SSE](/docs/storage/minio#server-side-encryption-sse) â€” configure the KMS key on the RustFS server process:
