...
The `@filenest-fs/nextjs` SDK provides server-side utilities for Next.js App Router â€” a pre-configured `FileNest` client, an upload token helper, and a webhook verification utility.

## Installation

<PackageTabs pkg="@filenest-fs/nextjs" />
...
`filenestServer()` has the same namespace API as `@filenest-fs/node` â€” see the [Node.js SDK](/docs/sdks/node) for full method docs.

---

## Upload token endpoint

Your app must expose a token endpoint that the `<FileNestProvider>` will call before each upload. The endpoint authenticates the user and issues a short-lived `fn_upload_token_...` â€” the API key stays server-side.
...
export async function POST(req: Request) {
  ...
  const token = await fn.uploadTokens.create({
    ...
    maxSize: 50 * 1024 * 1024,   // 50 MB â€” must be â‰¤ project config ceiling
    maxFiles: 10,
    expiresIn: 3600,
  ...
  // Return both token and expiresAt â€” the React SDK uses expiresAt to schedule refresh
  return Response.json({ token: token.token, expiresAt: token.expiresAt });
}
```

Point `<FileNestProvider tokenEndpoint="/api/filenest-token">` at this route â€” it will be called automatically before each upload.
...
The signing secret is the `signing_secret` returned when you create a webhook endpoint. It is shown only once â€” store it in an environment variable.
