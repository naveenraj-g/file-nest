...
The `@filenest/nextjs` SDK provides server-side utilities for Next.js App Router â€” a pre-configured `FileNest` client, an upload token helper, and a webhook verification utility.
...
`filenestServer()` has the same namespace API as `@filenest/node` â€” see the [Node.js SDK](/docs/sdks/node) for full method docs.
