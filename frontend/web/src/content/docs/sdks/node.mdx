---
title: "Node.js SDK"
description: "Install and use the @filenest-fs/node SDK for server-side file operations."
---

# Node.js SDK

The `@filenest-fs/node` SDK provides a typed client for all FileNest REST API operations. Use it in any Node.js runtime â€” Next.js server components, Express, Fastify, serverless functions, scripts.

## Installation

<PackageTabs pkg="@filenest-fs/node" />
...
Files under 5 MB are uploaded in a single request. Files over 5 MB are automatically split into multipart upload parts â€” no configuration needed.
...
await fn.files.delete(fileId);   // soft delete â€” file is recoverable
...
```typescript
// Create a single folder (parentFolderId=null â†’ project root)
const folder = await fn.folders.create({
  name: "invoices",
  parentFolderId: "fld_01j...",  // optional â€” omit for root-level folder
...
Issue short-lived tokens for browser-side uploads. The token is returned to the client through your own token endpoint â€” the API key never reaches the browser.
...
// token.token â†’ "fn_upload_token_..."  â€” return this to the browser
// token.expiresAt â†’ ISO timestamp â€” return this too so the browser can schedule refresh
```

Token constraints are validated against the project's config at creation time â€” if the project only allows `image/*`, passing `application/pdf` in `allowedMimeTypes` will return a 422 immediately. At upload time both the token constraints and the project config are enforced independently. See [Upload Tokens API reference](/docs/api/upload-tokens) for the full constraint reconciliation rules.
...
// webhook.signingSecret â€” store safely, shown only once
...
`fn.uploads` gives you manual control over the multipart session lifecycle. Use this when you need to resume an interrupted upload â€” for example, a large file upload that failed mid-way.
...
// session.uploadId â†’ persist this to resume later
