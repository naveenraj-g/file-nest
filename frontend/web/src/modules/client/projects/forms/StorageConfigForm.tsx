 ...
 *   - endpoint_url: required for MinIO, R2, RestFS; hidden for S3/Azure/GCS
...
const STATUS_CONFIG = {
  ...
  pending_verification: { icon: Clock, label: "Pending verification", className: "text-yellow-600" },
  verification_failed: { icon: AlertCircle, label: "Verification failed", className: "text-destructive" },
} as const;
...
export function StorageConfigForm({ projectId, config }: StorageConfigFormProps) {
  ...
  const { execute: save, isPending: isSaving } = useServerAction(updateStorageConfigAction, {
    onSuccess: () =>
  ...
  const { execute: verify, isPending: isVerifying } = useServerAction(verifyStorageAction, {
    onSuccess: ({ data }) => {
      if (data?.ok) {
  ...
  async function onSubmit(values: TStorageConfigForm) {
    ...
    await save({
      ...
      transportOptions: { shouldRevalidate: true, url: `/projects/${projectId}/settings` },
    });
  }
  ...
  if (config.storage_mode === "managed") {
    return (
      <div className="rounded-lg border bg-muted/40 p-5 text-sm text-muted-foreground">
        This project uses <strong className="text-foreground">managed storage</strong> —
        FileNest provisions and manages the bucket. No credentials are required.
      </div>
    );
  }
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div className={cn("flex items-center gap-2 text-sm font-medium", statusCfg.className)}>
      ...
      <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-5">
        <FormInput
          ...
          label={provider === "azure_blob" ? "Container name" : "Bucket name"}
        ...
        {needsEndpoint && (
          <FormInput
            ...
            control={form.control}
            label="Endpoint URL"
            description="Required for this provider"
            placeholder={
              provider === "r2"
        ...
        {!needsEndpoint && (
          <FormInput
            ...
            control={form.control}
            label="Region"
            description={provider !== "s3" ? "Optional" : undefined}
        ...
        <FormInput
          name="access_key_id"
          control={form.control}
          label={provider === "azure_blob" ? "Storage account name" : "Access key ID"}
          placeholder={provider === "azure_blob" ? "mystorageaccount" : "AKIAIOSFODNN7EXAMPLE"}
          autoComplete="off"
        />
        ...
        <FormInput
          ...
          control={form.control}
          label={provider === "azure_blob" ? "Storage access key" : "Secret access key"}
          type="password"
          placeholder="••••••••••••••••••••••••••••••••"
          autoComplete="new-password"
          description="Stored encrypted — never returned by the API after save."
        />
        ...
        {isS3 && (
          ...
          >
            <FormSelectOption value="AES256">AES-256 (default)</FormSelectOption>
            <FormSelectOption value="aws:kms">AWS KMS</FormSelectOption>
          </FormSelect>
        )}
        ...
        <div className="pt-1">
          <p className="text-xs text-muted-foreground mb-4">
            Provider:{" "}
            <span className="font-medium text-foreground">{PROVIDER_LABELS[provider]}</span>. After
            saving, click <strong>Verify connection</strong> to confirm access.
          </p>
